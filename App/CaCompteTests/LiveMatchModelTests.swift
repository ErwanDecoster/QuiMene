...
/// mémoire que `CaCompteKit/Tests/StoreTests` (`MatchRepositoryTests`), même catalogue de test
...
struct LiveMatchModelTests {
  ...
  /// `CaCompteKit/Tests/StoreTests`, où le conteneur reste une variable du corps du test, jamais
  ...
  private func makeModel(roundLimit: Int, scoreBounds: (min: Int, max: Int) = (0, 20)) throws -> (
    ...
    // `CaCompte.app` (`TEST_HOST`), qui porte l'entitlement iCloud réel. Sans ce réglage
    // explicite, `.automatic` (par défaut) tente quand même CloudKit dans ce process précis —
    // absent d'un exécutable de test non hébergé comme `CaCompteKit/Tests/StoreTests`.
