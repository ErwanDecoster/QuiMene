...
public struct LeaderboardRepository {
    ...
    public func leaderboard(for gameID: String) throws -> [LeaderboardEntry] {
        ...
        let participations = try context.fetch(FetchDescriptor<ParticipantRecord>())
            .filter { $0.player != nil && $0.match?.gameID == gameID && $0.match?.statusRaw == MatchStatus.ended.rawValue }
        ...
        return byPlayer.values
            ...
            .sorted { lhs, rhs in
                ...
                if lhs.averageNormalizedRank != rhs.averageNormalizedRank { return lhs.averageNormalizedRank > rhs.averageNormalizedRank }
