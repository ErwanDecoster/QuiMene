 ...
 *
 * **Découverte** : `cacompte_open_games` (Postgres, mêmes migrations `supabase/migrations` que
...
class SupabaseTransport(
     ...
     * `cacompte_open_games`, stable pour toute la durée de la session. `matchID`/`gameID`/
...
private suspend fun trySuspend(block: suspend () -> Unit) {
    try {
        ...
        // Best-effort : une ligne cacompte_open_games qui ne se supprime pas n'empêche rien
...
sealed class SupabaseTransportError(
    ...
    /** Aucune ligne `cacompte_open_games` pour ce code — code erroné, périmé (partie déjà
