...
///
/// **Découverte** : `quimene_open_games` (Postgres, migration `supabase/migrations`) résout un code
...
public final class SupabaseTransport {
  ...
  /// canal Realtime et la ligne `quimene_open_games`, stable pour toute la durée de la session.
  ...
  /// `quimene_open_games` directement voie une partie courante à jour. Ne touche ni au canal
  ...
  public func stopAdvertising() async {
    ...
    if let hostSessionID {
      _ = try? await client.rpc(
        "quimene_close_open_game", params: ["p_session_id": hostSessionID.uuidString]
      ).execute()
    }
    hostSessionID = nil
  }
  ...
  public func resolveGame(code: String) async throws -> DiscoveredHost {
    let row: OpenGameRow
    do {
      row = try await client.rpc("quimene_resolve_game", params: ["p_pairing_code": code])
        .single()
        .execute()
        .value
...
public enum SupabaseTransportError: Error, Sendable, Equatable {
  /// Aucune ligne `quimene_open_games` pour ce code — code erroné, périmé (partie déjà arrêtée), ou
  /// jamais existé.
  case gameNotFound
}
...
/// Paramètres de `quimene_advertise_game` — clés encodées = noms des paramètres SQL.
...
/// Paramètres de `quimene_update_open_game` (`updateActiveMatch`) — seules les colonnes qui
