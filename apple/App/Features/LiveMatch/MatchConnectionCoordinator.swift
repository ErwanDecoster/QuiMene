...
final class MatchConnectionCoordinator {
    ...
    -> Role
  {
    try await rejoin(
      PersistedSession(
        pairingCode: code, role: requestedRole, deviceName: deviceName, appVersion: appVersion))
  }
  ...
  private func rejoin(_ persisted: PersistedSession) async throws -> Role {
    ...
    let newModel = SharedMatchModel(session: liveSession, role: assignedRole, catalog: catalog) {
      [weak self] model in
